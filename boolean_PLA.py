...
def from_pla_to_expression(input_pattern, variables):
    """Convert a PLA input pattern to a Boolean expression."""
    return " & ".join([f"{variables[i]}" if bit == '1' else f"~{variables[i]}" for i, bit in enumerate(input_pattern)])


def main(input_filename, output_filename):
    output_data = []

    # Define the list of variables (up to 26 for simplicity)
    all_variables = [chr(i) for i in range(65, 91)]  # A-Z
    ...
    with open(input_filename, 'r') as infile:
        ...
        # Make sure the PLA file has single output
        if num_outputs != 1:
            raise ValueError("This program only supports single output logic functions.")

        # Take the required number of variables based on the number of inputs
        variables = all_variables[:num_inputs]

        minterms_list = []

        for line in lines[2:]:  # Skip the header lines
            input_pattern, output_value = line.strip().split()
            if output_value == "1":
                minterm = sum([(int(bit) << i) for i, bit in enumerate(reversed(input_pattern))])
                minterms_list.append(minterm)

        # Construct the original expression using the minterms
        expressions = [from_pla_to_expression(format(minterm, f"0{num_inputs}b"), variables) for minterm in minterms_list]
